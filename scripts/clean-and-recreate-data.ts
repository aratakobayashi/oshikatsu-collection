...
async function cleanAndRecreateData() {
  ...
  try {
    ...
    console.log('✅ Celebrity取得:', celebrity.id)
    
    // 1. episode_locations/episode_items を完全削除
    console.log('\n🗑️ リンクテーブル完全削除中...')
    await supabase.from('episode_locations').delete().gte('id', '00000000-0000-0000-0000-000000000000')
    await supabase.from('episode_items').delete().gte('id', '00000000-0000-0000-0000-000000000000')
    console.log('✅ リンクテーブル削除完了')
    
    // 2. locations/items を完全削除  
    console.log('\n🗑️ メインテーブル完全削除中...')
    await supabase.from('locations').delete().gte('id', '00000000-0000-0000-0000-000000000000')
    await supabase.from('items').delete().gte('id', '00000000-0000-0000-0000-000000000000')
    console.log('✅ メインテーブル削除完了')
    ...
    insertedLocations?.forEach((loc, idx) => {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  cleanAndRecreateData().catch(console.error)
}