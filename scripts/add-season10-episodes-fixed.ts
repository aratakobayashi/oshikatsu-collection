...
/**
 * 孤独のグルメ Season10 エピソードをデータベースに追加（修正版）
 * 正しいスキーマで実装
 */
...
async function addSeason10EpisodesFixed() {
  console.log('🎬 孤独のグルメ Season10 エピソード追加開始（修正版）...\n')
  ...
  try {
    ...
    for (const episode of seasonData.episodes) {
      ...
      console.log(`📝 追加中: 第${episode.episode_number}話`)
      ...
      const { data: episodeData, error: episodeError } = await supabase
        ...
        .insert({
          ...
          date: episode.air_date || null,  // air_date → date
          thumbnail_url: episode.still_path ? `${TMDB_IMAGE_BASE_URL}${episode.still_path}` : null,
          video_url: '',  // 空文字列でNOT NULL制約を回避
          duration: null,
          view_count: 0,
          like_count: 0,
          comment_count: 0,
          celebrity_id: null,  // 後で松重豊のIDを設定
      ...
      console.log(`  ✅ エピソード追加成功: ${episodeId}`)
      successCount++
    ...
    console.log(`🎉 Season10 エピソード追加完了！`)
    console.log(`✅ 成功: ${successCount}/${seasonData.episodes.length}エピソード`)
    console.log('=' .repeat(70))
    ...
    if (count === 12) {
      ...
      console.log(`⚠️ 一部エピソードが追加済み: ${count}/12`)
    }
...
// スクリプト実行
addSeason10EpisodesFixed()

export { addSeason10EpisodesFixed }
