...
async function updatePriorityRestaurants() {
  console.log('🍜 孤独のグルメ優先店舗データ更新開始（最終版）...\n')
  ...
  for (const update of PRIORITY_UPDATES) {
    ...
    try {
      ...
      // ロケーションデータを更新（実際のスキーマに合わせる）
      const updateData: any = {
        name: update.new_name,
        address: update.address,
        description: update.description,
        tabelog_url: affiliateUrl,  // 食べログアフィリエイトURL
        affiliate_info: {
          tabelog: {
      ...
      if (error) {
        ...
        console.log(`      - アフィリエイト: ${affiliateUrl.substring(0, 60)}...`)
        successCount++
      }
  ...
  if (successCount > 0) {
    ...
    console.log('2. 各エピソードの詳細ページ確認')
    console.log('3. ロケーション詳細ページで食べログボタン確認')
    console.log('4. アフィリエイトリンクのクリック動作確認')
    
    console.log('\n💡 確認方法:')
    console.log('npx tsx scripts/check-matsushige-data.ts')
  }
}
