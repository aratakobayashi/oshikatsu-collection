...
// Supabaseにデータを投入（修正版）
async function insertDataToSupabaseFixed(realData) {
  console.log('💾 修正版Supabaseデータ投入開始...')
...
// メイン処理
async function main() {
  console.log('🚀 修正版動的データ生成システム v3.0')
  console.log('===============================\n')
  ...
  if (!analysisFile) {
    ...
    console.log('💡 使用方法: node dynamic-data-generator-fixed.cjs <分析結果ファイル>')
    process.exit(1)
  }
  ...
  // 2. 処理結果をファイルに保存
  const outputFile = path.join(path.dirname(analysisFile), `fixed-dynamic-data-${Date.now()}.json`)
  fs.writeFileSync(outputFile, JSON.stringify(realData, null, 2), 'utf8')
  console.log(`📄 修正版データファイル作成: ${outputFile}`)
  
  // 3. Supabaseへのデータ投入
  console.log('\n💾 修正版Supabaseへのデータ投入を開始...')
  await insertDataToSupabaseFixed(realData)
  
  console.log('\n🎉 修正版データ生成・投入完了！')
  console.log('\n📋 次のステップ:')
  console.log('1. staging環境でエピソード詳細ページを確認')
  console.log('2. 修正版で生成されたデータが正しく表示されるかテスト')
...
module.exports = { processAnalysisResultsDynamic, insertDataToSupabaseFixed }