...
async function fixSeason4Episode8CorrectYohosCafeLanai() {
  ...
  console.log('間違った川豊（浅草うなぎ） → 正しいYO-HOs cafe Lanai（杉並区阿佐ヶ谷ハワイアン）')
  ...
  try {
    ...
    console.log('   - 実際のロケ地は「YO-HOs cafe Lanai」（杉並区阿佐ヶ谷・ハワイアン）')
    ...
    const correctedData = {
      ...
      affiliate_info: {
        linkswitch: {
          ...
          correction_note: '間違った川豊から正しいYO-HOs cafe Lanaiに修正済み（閉店のためLinkSwitch無効）',
    ...
    console.log('   After:  YO-HOs cafe Lanai（阿佐ヶ谷・ハワイアン・実際のロケ地）')
    console.log('   URL:    なし → 正しいYO-HOs cafe LanaiタベログURL')
